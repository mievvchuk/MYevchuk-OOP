...
namespace LR1_3
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                // 1. Зчитуємо дані з полів вводу
                string misto = textBox1.Text;
                double vidstan = double.Parse(textBox2.Text);
                int dni = int.Parse(textBox3.Text);
                string transport = comboBox1.SelectedItem.ToString();
                string typTuru = comboBox2.SelectedItem.ToString();

                // Створюємо змінну базового інтерфейсу для зберігання будь-якого типу подорожі
                IPodorozh p;

                // 2. Використовуємо switch для створення об'єкта відповідного класу
                switch (typTuru)
                {
                    case "Туристична":
                        // Для туристичної подорожі задаємо вартість проживання
                        double vartistProzhyvannya = 1000;
                ...
                // Додаємо створений об'єкт до загального списку
                podorozhi.Add((Podorozh)p);
                // 3. Готуємо дані для колонок, що залежать від типу подорожі
                ...
                // 4. Додаємо новий рядок у таблицю з усіма даними
                dataGridView1.Rows.Add(
                    p.Misto,
                    p.Vidstan,
                    p.Dni,
                    p.Transport,
                    p.RozrakhVartist(), // Базова вартість
                    p.SerednyaTsinaKm().ToString("F2"),
                    realTypTuru,
                    vartistProzhyvannyaStr, // Вартість проживання (тільки для туристичних)
                    zagalnaVartistStr       // Повна вартість (для всіх спеціальних типів)
                );

                // 5. Очищуємо поля для наступного вводу
                textBox1.Clear();
                textBox2.Clear();
                textBox3.Clear();
            }
            ...
            {
                // Якщо дані введено некоректно, показуємо повідомлення про помилку
